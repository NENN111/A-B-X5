...
## Быстрый запуск готового дашборда

Нужны Windows 10/11, Docker Desktop, PowerShell и Power BI Desktop. Python на
компьютере пользователя не требуется: расчёты выполняются в контейнере.

```powershell
git clone git@github.com:NENN111/A-B-X5.git
cd A-B-X5
powershell -ExecutionPolicy Bypass -File .\scripts\setup_windows.ps1
```

Сценарий запускает PostgreSQL, скачивает RetailHero, рассчитывает аналитику,
загружает девять витрин, проверяет результат и открывает готовый PBIP. Первый
запуск может занять 15–40 минут.

При запросе Power BI выберите аутентификацию **База данных**: сервер
`localhost:5432`, база `retail_experiments`, пользователь `retail`, пароль
`retail_local_only`. После ввода учётных данных нажмите **Обновить**.

Точка входа: [`powerbi/dashboard/RetailExperimentation.pbip`](powerbi/dashboard/RetailExperimentation.pbip).
Диагностика и ручной запуск: [`docs/deployment.md`](docs/deployment.md).
...
> Статус: **проект завершён.** Реализованы автоматическая загрузка RetailHero, data layer, feature mart, A/B-анализ, валидация эксперимента, uplift-модели, оптимизация таргетинга, PostgreSQL-витрины и готовый Power BI dashboard.
...
Готовый PBIP-проект содержит пять страниц отчёта:
...
- Исходные данные занимают около 620 МБ и скачиваются при первом запуске; они не хранятся в Git.
- Power BI Desktop доступен только на Windows. Сам аналитический конвейер и PostgreSQL запускаются через Docker на любой поддерживаемой им ОС.
- Стандартные Docker credentials предназначены только для локальной демонстрации и должны быть заменены при публикации во внешней среде.
- Денежные показатели являются сценарными и зависят от заданных стоимости коммуникации и прибыли с конверсии.
